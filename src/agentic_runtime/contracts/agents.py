"""Definiciones de subagente — T1 invariante (`SEAMS §S25`, `05·E28`).

Homologación de `loadAgentsDir.AgentDefinition` + `utils/model/agent.getAgentModel`
del canónico.
...
class AgentDefinition:
    ...
    # --- campos de EJECUCIÓN (`SEAMS §S25`, `05·E28`/`E16`/`E22`/`E20`) ---
    #: Tope de turnos del subagente (`05·E16`; peer de `RuntimeTask.max_turns`, `01·CTR-14`).
    max_turns: int | None = None
    #: Force-async del subagente (`05·E22` → `S22`, cuya política queda fuera del tramo).
    background: bool = False
    #: Denylist. Hasta hoy sólo existía allowlist, luego no había forma de restar una tool.
    disallowed_tools: frozenset[str] = frozenset()
    #: Override de modo del hijo (`05·E20` → `S17`); `None` = hereda el del padre.
    permission_mode: PermissionMode | None = None
    # DELEGADOS que NO viven aquí (`SEAMS §S25`): mcp_servers→11 · skills/hooks→12/06 ·
    # effort→16 · memory→13 · isolation→10/18. Dos de los seis se comprobaron huérfanos;
    # los cuatro restantes están **sin comprobar** y se comprueban al reconciliar su par.
