...
def test_background_on_complete_returns_true():
    mgr = ModeManager()
    mgr.register(execution_id="t1", mode=AgentMode.BACKGROUND)
    assert mgr.on_complete("t1") is True


def test_foreground_on_complete_returns_false():
    mgr = ModeManager()
    mgr.register(execution_id="t1", mode=AgentMode.FOREGROUND)
...
def test_fork_mode_is_immutable():
    mgr = ModeManager()
    mgr.register(execution_id="t1", mode=AgentMode.FORK)
