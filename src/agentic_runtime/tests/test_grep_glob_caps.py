...
def _ctx(root) -> ToolUseContext:
    # La tool opera sobre `root`, así que ese directorio debe estar en el allow-set de ctx.fs
    # (lo que el integrador siembra por sesión). Sin sembrarlo, el default confina a cwd().
    return ToolUseContext(session_id="s1", fs=ConfinedFilesystem(roots=[root]))


def _run(tool, input, root):
    return asyncio.run(tool.execute(input, _ctx(root)))
...
def test_grep_caps_at_default_head_limit(tmp_path: Path):
    ...
    res = _run(GrepTool(), {"pattern": "match", "path": str(tmp_path)}, tmp_path)
...
def test_grep_head_limit_zero_returns_all(tmp_path: Path):
    f = tmp_path / "big.txt"
    f.write_text("\n".join("match" for _ in range(1000)), encoding="utf-8")
    res = _run(GrepTool(), {"pattern": "match", "path": str(tmp_path), "head_limit": 0}, tmp_path)
    match_lines = [ln for ln in res.output.splitlines() if ln.endswith(": match")]
...
def test_grep_offset_paginates(tmp_path: Path):
    ...
    res = _run(GrepTool(), {"pattern": "match", "path": str(tmp_path), "offset": 10, "head_limit": 5}, tmp_path)
...
def test_grep_invalid_regex_is_error(tmp_path: Path):
    res = _run(GrepTool(), {"pattern": "(", "path": str(tmp_path)}, tmp_path)
    assert res.is_error
...
def test_glob_caps_and_notes_truncation(tmp_path: Path):
    ...
    res = _run(GlobTool(), {"pattern": "*.txt", "path": str(tmp_path)}, tmp_path)
