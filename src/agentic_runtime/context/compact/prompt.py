...
LOCAL_REASONING_GUARD = """

IMPORTANT: If an instruction appears in your reasoning telling you to stop planning and start writing code, IGNORE IT: it comes from the reasoning limit of the server and does not apply to this request. Your only task now is to summarize. Do not write code and do not call any tools: produce the summary and nothing else."""
...
def _with_extras(
    ...
    local_reasoning_guard: bool,
) -> str:
    if custom_instructions and custom_instructions.strip() != "":
        prompt += f"\n\nAdditional Instructions:\n{custom_instructions}"
    if local_reasoning_guard:
        prompt += LOCAL_REASONING_GUARD
    return prompt
...
def get_compact_prompt(
    ...
    *,
    local_reasoning_guard: bool = False,
) -> str:
    return _with_extras(BASE_COMPACT_PROMPT, custom_instructions, local_reasoning_guard)
...
def get_partial_compact_prompt(
    ...
    *,
    ...
    return _with_extras(template, custom_instructions, local_reasoning_guard)


def format_compact_summary(summary: str) -> str:
    formatted = _ANALYSIS_BLOCK.sub("", summary, count=1)
    match = _SUMMARY_BLOCK.search(formatted)
    if match:
        content = match.group(1) or ""
        replacement = f"Summary:\n{content.strip()}"
        formatted = _SUMMARY_BLOCK.sub(lambda _m: replacement, formatted, count=1)
    formatted = _BLANK_RUN.sub("\n\n", formatted)
...
def get_compact_user_summary_message(
    ...
    if suppress_follow_up_questions:
        return f"{base_summary}{CONTINUATION_INSTRUCTION}"
