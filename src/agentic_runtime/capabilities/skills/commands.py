...
def process_slash_command(
    ...
    Si `text` es `/<skill> args` y `<skill>` existe, activa la skill en `ctx`
    (mismo efecto que invocar la tool `Skill`: estado activo + allowed-tools) y
    devuelve sus instrucciones renderizadas. Devuelve None si no es un slash command
    o la skill no existe — el integrador lo trata como prompt normal.
