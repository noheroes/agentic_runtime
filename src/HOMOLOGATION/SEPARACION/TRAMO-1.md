...
- **⚠ NO pagado, nombrado entero (`L07`):** `NativeToolRegistry` no tiene **ningún** call-site de producción (sólo `__init__` y un test) — un tercer registro conviviendo con `ToolRegistry` y `ToolPool`, exactamente la forma del doble-camino que `C7` cerró para `S19`. Y `ToolRegistry.list_available(permission_ctx=…)` es **parámetro muerto**: los dos únicos llamadores (`agent_loop.py:128`, `capabilities/resolver.py:46`) pasan sólo `mode`. Ninguno de los dos cambia comportamiento hoy; retirar símbolo exportado excede el alcance nombrado de `C5` y se difiere **entero y nombrado**.
...
| E2 | turno real con **tool nativa real** (bash + fs), resultado aplanado, re-entrada | C4·C5·C6 | 🟢 2 tests (turno real `bash`+`write_file` con token que sólo vive en disco · invariante del **pool único** sobre el ctx de producción) | `add_numbers` no ejercita confinamiento |
...
| E7 | **confinamiento**: traversal/symlink/allow-set rechazados | C6 | 🟢 3 tests (mecanismo: traversal · symlink · allow-set r/w asimétrico · regresión `FIND-C6-1` · costura `S15` load-bearing · **negativa E2E real** con control positivo) | promueve C6 de G2 a G1 |
...
**Estado del gate (2026-08-01, 5ª ventana):** `uv run pytest src/agentic_runtime/tests/test_tramo1_gate.py
-m gate_tramo1 -q` = **17 passed, 0 skipped, en una sola corrida** (`E1`×2 · `E2`×2 · `E3`×1 · `E4`×4 · `E5`×1 ·
`E6`×3 · `E7`×3 · `E9`×1). **8 de 9 — falta sólo `E8`. El tramo NO está cerrado.** Deuda de cierre re-medida en
esta ventana (nada heredado): `mypy --strict` **139 err / 55 f** (sin cambio) · `ruff` **502** (+2, ambos
calcados del idioma inmediato del propio módulo: `RUF012` espeja `IdentityWitnessTool`, `UP037` espeja
`_default_pool`/`_default_fs`) · suite **695 passed / 3 skipped / 111 xfailed / 0 failed**.

El delta de suite cuadra entero contra la medición anterior (688 / 112): **+7 passed** = las 5 del gate nuevas
+ `test_tool_result_carries_ends_turn_as_declared_b_extension` + el `xfail` volteado; **−1 xfailed** por ese
mismo volteo. Ningún `xfail` estricto pasó en silencio (cero `XPASS` en la corrida).
