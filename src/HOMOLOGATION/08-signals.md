...
| S2 | Propagación a la llamada al modelo (`signal`→fetch/stream) | `complete(stop=…)`→`replace(opts, signal=stop)`→`agentic_models` | ~~✅~~ **❌** | ⚠ **CORREGIDO 2026-07-29 en `SEPARACION/A-CIERRE-P4.md §14.2`.** El gate-11 siguió el dato hasta el punto de ENTREGA (`StreamOptions.signal`) y paró. Al abrir el **consumidor**: los 8 providers de `agentic_models` gatean el corte en `getattr(signal,"aborted",False)` y `caller.py:151/166` entrega un `asyncio.Event`, que no tiene `.aborted` ⇒ **el abort se ignora en todos**. `is_set()` en `agentic_models` = 0. Seam **roto en el tipo**, no "pendiente de verificar en 16" (16 ya dictaminó: `FIND-MODELS4`). Ver `SEPARACION/08-signals.md·CG-SIG-10`. |
...
| S12 | Error de abort a nivel API (`APIUserAbortError`, `stream.controller.abort()` cleanup) | delegado a `agentic_models` | ~~🔀~~ **❌** | ⚠ **CORREGIDO 2026-07-29** (ídem S2). No era delegable: `16·FIND-MODELS4` ya había dictaminado en contra ("abort roto, tipo de señal equivocado"); `SEAMS §S2` = `existe-roto`. Un `🔀` no puede delegar en una categoría que ya resolvió el punto. |
