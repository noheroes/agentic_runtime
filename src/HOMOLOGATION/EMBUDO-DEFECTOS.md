...
| wvw2_5, wvw3_5 | `WebSearch` | adenda a `D-35`: cláusula de vecindario en `bash` («Retrieve a web page: Use WebFetch») probada y RETIRADA. Scraper real 1/20 → 2/20, o sea ruido. `wvw2_5` se va a `pypi.org/pypi/httpx/json` y a `raw.githubusercontent.com` generalizando la salvaguarda del `gh` que la propia cláusula escribía; `wvw3_5` monta un `HTMLParser` completo y la cláusula no opera. Tercera refutación de la palanca redactada en una `description` tras `D-31` y `D-34` | descripción |
