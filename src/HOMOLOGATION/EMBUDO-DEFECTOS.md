...
| wzw2, wzw3, wzw4 | `WebSearch` | en esos mismos casos de grado 2, el snippet no basta y en 12 de 20 rondas el modelo baja a `bash` a escribir scrapers ad-hoc (`urllib` + `HTMLParser` a mano) sobre las URL devueltas, en vez de pedirlas por `WebFetch`. A cierra ahí porque su resultado trae el contenido redactado por el modelo interno del propio tool; B devuelve enlaces. Divergencia de mecanismo, en pie | resultado |
